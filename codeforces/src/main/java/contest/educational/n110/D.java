package contest.educational.n110;
...
public class D {
  ...
  public static void main(String[] args) {
    ...
    int k = scan.scanInt();
    char[] data = scan.scan1dCharArray();
    int q = scan.scanInt();
    String[][] queries = scan.scan2dStringArray(q, 2);
    map = new HashMap<>();

    Node root = createTree(data, 0, null);

    for (String[] query : queries) {
      updateTree(map.get(Integer.parseInt(query[0]) - 1), query[1].toCharArray()[0]);

      print.printLine(Integer.toString(root.possible));
    }
  ...
  private static void updateTree(Node node, char c) {
    if (c == '?') {
      node.fixed = false;
      node.possible = node.leaf ? 2 : node.left.possible + node.right.possible;

      while (node.parent != null && !node.parent.fixed) {
        node = node.parent;
        node.possible = node.leaf ? 2 : node.left.possible + node.right.possible;
      }
    } else {
  ...
  private static Node createTree(char[] data, int index, Node parent) {
    int i = data.length - 1 - index;
    if (i < 0) {
      return null;
    }
    ...
    n.parent = parent;
    n.left = createTree(data, 2 * index + 2, n);
    n.right = createTree(data, 2 * index + 1, n);
    ...
    if (n.left == null && n.right == null) {
      ...
      if (data[i] == '?') {
        n.fixed = false;
        n.possible = n.left.possible + n.right.possible;
      } else {
        n.fixed = true;
        n.possible = 1;
      }
