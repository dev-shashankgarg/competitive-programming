...
public class C {
  ...
  public static void main(String[] args) {
    ...
    IntStream.range(0, t).forEach(test -> {
      ...
      print.printLine(Long.toString(solve(data)));
    });
  ...
  private static long solve(char[] data) {
    int i = 0;
    int j = 0;
    ...
    while (j < data.length) {
      if (data[j] == '?') {
        if (i != j) {
          li.add(new Node(data[i], data[j - 1], j - i));
          i = j;
        }
    ...
    for (int k = 0; k < li.size(); k++) {
      if (li.get(k).l == '?') {
        if (k == 0) {
          if (li.size() == 1) {
            long n = li.get(k).size;
            return (n * (n + 1)) / 2;
          } else {
            li.get(k + 1).size += li.get(k).size;
          }
        } else if (k == li.size() - 1) {
          li.get(k - 1).size += li.get(k).size;
        } else {
          long n = li.get(k).size;
          if ((n % 2 == 1 && li.get(k - 1).r == li.get(k + 1).l) ||
              (n % 2 == 0 && li.get(k - 1).r != li.get(k + 1).l)) {
            li.get(k + 1).size += (li.get(k - 1).size + li.get(k).size);
            li.get(k - 1).size = 0;
          } else {
            li.get(k - 1).size += (li.get(k).size);
            li.get(k + 1).size += (li.get(k).size);

            long nn = li.get(k).size;
            ans -= ((nn * (nn + 1)) / 2);
          }
        }
      }
  ...
  static class Node {
    ...
    public Node(char l, char r, int size) {
      this.l = l;
      this.r = r;
      this.size = size;
    }
